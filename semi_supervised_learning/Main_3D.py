...
from Model_3D import Unet, load_callback_list
from Data_3D import load_data, print_func, save_results
...
def train_model(data_path=data_path, imgs="mr_bffe.mhd", msks="prostaat.mhd", model_name="model", save_path = "results", num_folds=5, batch_size=32, learning_rate=1e-5, nr_epochs=50, verbosity=1, up=False, start_ch=32, depth=4, inc_rate=2, kernel_size=(3, 3, 3), activation='relu', normalization=None, dropout=0):
    
    ##### load data and optional test data #####
    images, masks = load_data(data_path, imgs, msks, 96, 96, 48)
    
    ##### save arguments for the model to dictionairy #####
    arg_dict_model = {"img_shape": (48, 96, 96, 1), "start_ch": start_ch, "depth": depth, "inc_rate": inc_rate, "kernel_size": kernel_size, "activation": activation, "normalization": normalization, "dropout": dropout, "learning_rate": learning_rate, "up": up}
