...
def reshape_imgs(imgs, img_x, img_y, img_z):
    """
    ...
    imgs_p = np.ndarray((imgs.shape[0], img_z, img_x, img_y), dtype=np.uint8)
    for i in range(imgs.shape[0]):
        imgs_p[i] = resize(imgs[i], (img_z, img_x, img_y), preserve_range=True)

    imgs_p = imgs_p[..., np.newaxis]
    return imgs_p

def load_data(data_path, imgs="mr_bffe.mhd", msks="prostaat.mhd", img_x=272, img_y=320, img_z=80):
    ##### loading the data to numpy arrays #####
    print_func('Loading Data')
    patients = os.listdir(data_path)
    imgs_train = np.asarray([sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(data_path, patient, "mr_bffe.mhd"))) for patient in patients if patient.find("p1")>-1])
    msks_train = np.asarray([sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(data_path, patient, "prostaat.mhd"))) for patient in patients if patient.find("p1")>-1])
    
    print_func(f"Reshape images to shape {img_z}x{img_x}x{img_y}")
    imgs_train = reshape_imgs(imgs_train, img_x, img_y, img_z)
    msks_train = reshape_imgs(msks_train, img_x, img_y, img_z)
